...
def find_thetaset(n,k,d,is_strict): #set is_strict to 1 or 0 as desired
    ...
    raising_ops = [] 
    if n <= memory_limit:
        partitions = make_partitions(n,k+1,is_strict)     
        for p in partitions:
            raising_ops.append(vectors(p,k,d)) 
        raising_ops = remove_duplicates(raising_ops)
        return raising_ops
    else:
        olddiff = difference_list(memory_limit-1,k+1,is_strict) #change to 0 for nonstrict
        newdiff = difference_list(n-memory_limit,k+1,is_strict) #again, change 0 if needed
#        print olddiff
#        print newdiff
        for new in newdiff:
            newolddiff = []
...
def thetasuperset_equiv(superset1,superset2):
    nonmatch1 = []
    nonmatch2 = []
    for thetaset1 in superset1:
        if thetaset1 not in superset2:
            nonmatch1.append(thetaset1)
    for thetaset2 in superset2:
        if thetaset2 not in superset1:
            nonmatch2.append(thetaset2)
    if nonmatch1 == [] and nonmatch2 == []:
        print "The supersets are equivalent"
    return nonmatch1,nonmatch2
